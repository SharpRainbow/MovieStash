        ...
        binding.userColRv.adapter = CollectionAdapter(cols, false){
            ...
            lifecycleScope.launch {
                when(val result: Result<Boolean> = DatabaseController.isModerator()) {
                    ...
                    is Result.Error -> Toast.makeText(this@UserCollectionActivity,
    ...
    private fun refresh(){
        ...
        lifecycleScope.launch {
            when(val result: Result<Boolean> = DatabaseController.checkConnection()) {
                is Result.Success<Boolean> -> {
                    if (!result.data){
                        Toast.makeText(this@UserCollectionActivity,
            ...
            DatabaseController.user?.let {
            when (val result : Result<ResultSet> = DatabaseController.getUserCols(it.uid)) {
                is Result.Success<ResultSet> -> {
                    result.data.let { set ->
                        prevSize = cols.size
                        while (set.next()){
                            val tmp = Collection(set.getInt("collection_id"),
                                set.getString("name"), it.uid, set.getString("description"))
                            cols.add(tmp)
                        }
                        if (cols.size - prevSize == 1)
                            binding.userColRv.adapter?.notifyItemInserted(cols.size)
                        else if (cols.size - prevSize > 1)
                            binding.userColRv.adapter?.notifyItemRangeInserted(0, cols.size)
                        else
                            Toast.makeText(this@UserCollectionActivity, "Ничего не найдено", Toast.LENGTH_SHORT).show()
                    }
                }
                is Result.Error -> {
                    Toast.makeText(
