...
class ReviewEditorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        intent.getParcelableExtra<Review>("RV")?.let {
            ...
            if (it.opinion.contains("негативный"))
                binding.opSpinner.setSelection(0)
            else if (it.opinion.contains("нейтральный"))
                binding.opSpinner.setSelection(1)
            else if (it.opinion.contains("позитивный"))
                binding.opSpinner.setSelection(2)
        }
    }
            ...
            lifecycleScope.launch {
                when(val result: Result<Boolean> = DatabaseController.checkConnection()) {
                    is Result.Success<Boolean> -> {
                        if (!result.data){
                            Toast.makeText(this@ReviewEditorActivity,
                ...
                val result: Result<Boolean> =
                    if (::review.isInitialized)
                        DatabaseController.addModReview(
                            titleText, descText, review.id,
                            opinion, true
                        )
                    else
                        DatabaseController.addModReview(
                            titleText, descText, filmID,
                            opinion, false
                        )
                when(result) {
                    is Result.Success<Boolean> -> {
                    ...
                    is Result.Error -> Toast.makeText(this@ReviewEditorActivity,
                        result.exception.message, Toast.LENGTH_SHORT).show()
                }
            }
        }
